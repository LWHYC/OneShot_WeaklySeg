...
def create_sample(image_path, label_path, coarseg_path=False, distance_path=False, class_wanted=[0, 1], transpose=True):
    ...
    oh_label = np.zeros([len(class_wanted)]+list(label.shape)).astype(np.int16)
    oh_label[0]=1
    for i in range(len(class_wanted)):
        if class_wanted[i] !=0:
            oh_label[i][label==class_wanted[i]]=1
            oh_label[0][label==class_wanted[i]]=0
    oh_label = torch.from_numpy(oh_label.astype(np.uint8))
    sample={'image':image,'label':oh_label, 'patient_path':image_path}
    if coarseg_path:
        coarseg = load_nifty_volume_as_array(coarseg_path, transpose=transpose).astype(np.int16)
        oh_coarseg = np.zeros([len(class_wanted)]+list(coarseg.shape)).astype(np.int16)
        oh_coarseg[0]=1
        for i in range(len(class_wanted)):
            if class_wanted[i] !=0:
                oh_coarseg[i][coarseg==class_wanted[i]]=1
                oh_coarseg[0][coarseg==class_wanted[i]]=0
        oh_coarseg = torch.from_numpy(oh_coarseg.astype(np.uint8))
        sample['coarseg']=oh_coarseg
    if distance_path:
...
class TorchioDataloader(Dataset):
    def __init__(self, config=None, class_wanted=3, split='train', num=None, transform=None, 
                ...
                if len(self.image_fold_ls)>0:
                    for image_fold in self.image_fold_ls:
                        ...
                        for i in tqdm(range(len(self.image_name_list))):
                            ...
                            if self._distance_filename:
                                ...
                                distance_path = False
                            image_task_dic[image_fold+'_'+self.image_name_list[i]] = p.apply_async(create_sample, args=(image_path, label_path, coarseg_path, distance_path, class_wanted, transpose, ))
                        p.close()
                        p.join()
                else:
                    ...
                    for i in tqdm(range(len(self.image_name_list))):
                        ...
                        if self._distance_filename:
                            ...
                            distance_path = False
                        image_task_dic[self.image_name_list[i]] = p.apply_async(create_sample, args=(image_path, label_path, coarseg_path, distance_path, class_wanted, ))
                    p.close()
                    p.join()
                    ...
                    for image_fold in self.image_fold_ls:
                        ...
                        if self._coarseg_filename:
                            for i in tqdm(range(len(self.image_name_list))):
                                ...
                                oh_coarseg = np.zeros([len(class_wanted)]+list(coarseg.shape)).astype(np.int16)
                                oh_coarseg[0]=1
                                for ii in range(len(class_wanted)):
                                    if class_wanted[ii] !=0:
                                        oh_coarseg[ii][coarseg==class_wanted[ii]]=1
                                        oh_coarseg[0][coarseg==class_wanted[ii]]=0
                                oh_coarseg = torch.from_numpy(oh_coarseg.astype(np.uint8))
                                self.image_dic[image_fold+'_'+self.image_name_list[i]] = {'coarseg':oh_coarseg}
                else:
                    self.image_name_list = os.listdir(self.folder_ls)
                    if self._coarseg_filename:
                        for i in tqdm(range(len(self.image_name_list))):
                            ...
                            for ii in range(len(class_wanted)):
                                if class_wanted[ii] !=0:
                                    ...
                                    oh_coarseg[0][coarseg==class_wanted[ii]]=0
                            oh_coarseg = torch.from_numpy(oh_coarseg.astype(np.uint8))
                            #print(oh_coarseg[0,0,0], oh_coarseg[1,0,0])
                            self.image_dic[self.image_name_list[i]] = {'coarseg':oh_coarseg}
                    print(self.image_dic.keys())
            if num is not None:
    ...
    def __getitem__(self, idx):
        if self.load_memory:
            ...
            if self._distance_filename:
                ...
                distance_path = False
            sample = create_sample(image_path, label_path, coarseg_path, distance_path, self.class_wanted)
        if self._coarseg_filename:
            sample['coarseg'] = self.image_dic[image_name]['coarseg']
        if self.transform:
            sample = self.transform(sample)
        return sample

    def get_specific_data(self, image_name):
        image_path = os.path.join(self.folder_ls, image_name, self._image_filename)
        image = torch.from_numpy(load_nifty_volume_as_array(image_path)[np.newaxis,:])

        label_path = os.path.join(self.folder_ls, image_name, self._label_filename)
        label = load_nifty_volume_as_array(label_path).astype(np.int16)
        oh_label = np.zeros([len(self.class_wanted)]+list(label.shape)).astype(np.int16)
        oh_label[0]=1
        for i in range(len(self.class_wanted)):
            if self.class_wanted[i] !=0:
                oh_label[i][label==self.class_wanted[i]]=1
                oh_label[0][label==self.class_wanted[i]]=0
        oh_label = torch.from_numpy(oh_label.astype(np.uint8))
        oh_coarseg = self.image_dic[image_name]['coarseg']
        return {'image':image, 'coarseg':oh_coarseg, 'label': oh_label}

    def update_corrupted_label(self, prob, label, coarseg, noise_thresh):
        ncoarseg = np.copy(coarseg)
        ncoarseg[0][prob[0]>noise_thresh[0]]=1
        ncoarseg[1][prob[1]>noise_thresh[1]]=1
        ncoarseg[1][prob[0]>noise_thresh[0]]=0
        ncoarseg[0][prob[1]>noise_thresh[1]]=0
        ncoarseg[ncoarseg!=coarseg]=0
        change = ncoarseg.astype(np.int16)-coarseg.astype(np.int16)
        print('changed',np.sum(change[0]>0), np.sum(change[0]<0),np.sum(change[1]>0),np.sum(change[1]<0))
        print('correctly changed', np.sum((change[0]>0)*(label[0]>0)), np.sum((change[1]>0)*(label[1]>0)))
        ncoarseg = torch.from_numpy(ncoarseg)
        return ncoarseg
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if cshape[0] <= self.output_size[0] or cshape[1] <= self.output_size[1] or cshape[2] <= \
            ...
            for key in sample.keys():
                if torch.is_tensor(sample[key]):
                    if key in ['label', 'coarseg']:
                        n0 = F.pad(sample[key][0:1], (pd, pd, ph, ph, pw, pw), mode='constant', value=1)
                        n1 = F.pad(sample[key][1::], (pd, pd, ph, ph, pw, pw), mode='constant', value=0)
                        sample[key] = torch.cat((n0, n1), dim=0)
                    else:
...
class RandomAffine(object):
    def __init__(self, scales=[0.2,0.2,0.2], degrees=[10,10,10], include=['image', 'label'], prob=0):
        self.prob = prob
        self.add_affine = tio.RandomAffine(
            scales=scales,
            degrees=degrees,
            include=include)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if 'coarseg' in sample:
            sample['oh_coarseg']=torch.round(sample['coarseg'].float())
