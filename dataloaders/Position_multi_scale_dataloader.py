...
class PositionDoublePatientDataloader(Dataset):
    """ structseg Dataset position """
    def __init__(self, config=None, split='train', num=None, transform=None, load_aug=False):
        self._data_root = config['data_root']
        self._image_filename = config['image_name']
        self._iternum = config['iter_num']
        self.split = split
        ...
        if load_aug:
            ...
            self._aug_cor_y_filename = config['aug_cor_y']
            self._aug_cor_z_filename = config['aug_cor_z']
        if split:
            self.image_name_list = os.listdir(self._data_root+'/'+split)
        else:
            ValueError('please input choose correct mode! i.e."train" "valid" "test"')
        # for i in trange(len(self.image_name_list)):
        #     image_path = os.path.join(self._data_root, self.split, self.image_name_list[i], self._image_filename)
        #     image, spacing = load_nifty_volume_as_array(image_path, return_spacing=True)
        #     spacing = np.asarray(spacing)
        #     sample = {'image': image, 'spacing':spacing, 'image_path':image_path}
        #     self.image_dic_list.append(sample)
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            #double_image_path = [self.image_name_list[idx], self.image_name_list[7]]
            double_image_path = [self.image_name_list[idx], self.image_name_list[0]]
            #double_image_path = [ self.image_name_list[idx], '/home/disk/LWH/Data/Pancreas/test/Pancreas071']
        if self._image_filename:
            image_path_0 = os.path.join(self._data_root, self.split, double_image_path[0], self._image_filename)
            image_path_1 = os.path.join(self._data_root, self.split, double_image_path[1], self._image_filename)
        else:
            image_path_0 = os.path.join(self._data_root, self.split, double_image_path[0])
            image_path_1 = os.path.join(self._data_root, self.split, double_image_path[1])
        ...
        if self.load_aug:
            aug_image_path_0 = os.path.join(self._data_root, self.split, double_image_path[0], self._aug_image_filename)
            aug_image_path_1 = os.path.join(self._data_root, self.split, double_image_path[1], self._aug_image_filename)
            aug_cor_x_path_0 = os.path.join(self._data_root, self.split, double_image_path[0], self._aug_cor_x_filename)
            aug_cor_x_path_1 = os.path.join(self._data_root, self.split, double_image_path[1], self._aug_cor_x_filename)
            aug_cor_y_path_0 = os.path.join(self._data_root, self.split, double_image_path[0], self._aug_cor_y_filename)
            aug_cor_y_path_1 = os.path.join(self._data_root, self.split, double_image_path[1], self._aug_cor_y_filename)
            aug_cor_z_path_0 = os.path.join(self._data_root, self.split, double_image_path[0], self._aug_cor_z_filename)
            aug_cor_z_path_1 = os.path.join(self._data_root, self.split, double_image_path[1], self._aug_cor_z_filename)
            aug_image_0 = load_nifty_volume_as_array(aug_image_path_0, return_spacing=False)
            aug_image_1 = load_nifty_volume_as_array(aug_image_path_1, return_spacing=False)
            aug_cor_x_0 = load_nifty_volume_as_array(aug_cor_x_path_0, return_spacing=False)
            aug_cor_x_1 = load_nifty_volume_as_array(aug_cor_x_path_1, return_spacing=False)
            aug_cor_y_0 = load_nifty_volume_as_array(aug_cor_y_path_0, return_spacing=False)
            aug_cor_y_1 = load_nifty_volume_as_array(aug_cor_y_path_1, return_spacing=False)
            aug_cor_z_0 = load_nifty_volume_as_array(aug_cor_z_path_0, return_spacing=False)
            aug_cor_z_1 = load_nifty_volume_as_array(aug_cor_z_path_1, return_spacing=False)
            sample['aug_image_0'] = aug_image_0
            sample['aug_image_1'] = aug_image_1
            sample['aug_cor_x_0'] = aug_cor_x_0
            sample['aug_cor_x_1'] = aug_cor_x_1
            sample['aug_cor_y_0'] = aug_cor_y_0
            sample['aug_cor_y_1'] = aug_cor_y_1
            sample['aug_cor_z_0'] = aug_cor_z_0
            sample['aug_cor_z_1'] = aug_cor_z_1
