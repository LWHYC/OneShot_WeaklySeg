...
class PRNet(Module):
    ...
    def forward(self, x, loss_func):
        ...
        feature0 = out[:, :, out.shape[2]//2, out.shape[3]//2,out.shape[4]//2]
        ...
        up8 = self.upsample(out)
        out = t.cat((up8, conv0), 1)
        out = self.conv8_1(out)
        out = self.conv8_2(out)
        feature1 = out[:, :, out.shape[2]//2, out.shape[3]//2,out.shape[4]//2]

        out = self.classification(feature1)
        dic = {'fc_position': fc_out}

        if loss_func != None:
            loss = loss_func(t.sigmoid(out), x)
            dic['loss'] = loss
        else:
            dic['ae'] = out
            dic['feature0'] = feature0
            dic['feature1'] = feature1
