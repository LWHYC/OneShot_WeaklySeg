...
def extract_label(file_path, file_name_ls, label_ls):
    ...
    for clabel in label_ls:
        ...
        nlabel = morphology.binary_erosion(nlabel, np.ones([2,3,3])).astype(np.int16)
        nlabel = get_largest_component(nlabel)
        nlabel = morphology.binary_dilation(nlabel, np.ones([2,3,3])).astype(np.int16)
...
p = Pool(12)
root_path = '../../../Data/HaN_structseg'
file_name_ls = ['geo_expdis_back.nii.gz', 'geo_expdis_brainstem.nii.gz', 'geo_expdis_leftparotid.nii.gz', 'geo_expdis_rightparotid.nii.gz']
label_ls = [1, 6, 7]
