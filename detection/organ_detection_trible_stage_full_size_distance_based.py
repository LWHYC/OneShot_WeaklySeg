...
def test(config_file):#, label_wanted):
    ...
    Coarse_RD = Relative_distance(Coarse_Pnet,out_mode='fc_position', feature_refine=False, distance_mode='tanh', \
                center_patch_size=[2,8,8], distance_ratio=coarse_dis_ratio)
    Middle_RD = Relative_distance(Middle_Pnet,out_mode='fc_position',feature_refine=False,distance_mode='tanh', \
                center_patch_size=[2,8,8], distance_ratio=middle_dis_ratio)
    Fine_RD = Relative_distance(Fine_Pnet,out_mode='fc_position',distance_mode='tanh', \
    ...
    for ognb in [1]:
        ...
        with torch.no_grad():
            ...
            for ii_batch, sample_batch in enumerate(validLoader):
                ...
                # label_0 = pad(load_volume_as_array(sample_batch['image_path_0'][0].replace('source_training', 'source_training_label2').replace('ceT1', 'Label')), max_scale)
                # ss = label_0.shape
                # label_1 = pad(load_volume_as_array(sample_batch['image_path_1'][0].replace('source_training', 'source_training_label2').replace('ceT1', 'Label')),max_scale)
                ...
                for move_step in range(iter_move_num[0]):
                    ...
                    middle_query_batch = []
                    cur_cor = np.around(cur_position/spacing).astype(np.int16) #像素坐标[6&2,3]
                    ...
                    relative_position = Middle_RD.cal_RD(query_patch=middle_query_batch)['relative_position']
                ...
                for move_step in range(iter_move_num[1]):
                    ...
                    cur_position = np.asarray(cur_position)+ relative_position # [6,3]
...
if __name__ == '__main__':
    config_file = str('/media/SENSETIME\wangna2/3bad972f-94e4-44c6-9534-b637a737e8e0/Code/RDR/config/test_position_full_size_trible_stage_style_panc.txt')
