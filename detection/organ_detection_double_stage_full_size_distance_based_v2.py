...
sys.path.append('/home/oem/home/data/zhaoqianfei/Relative_Distance_Regression/Relative_Distance_Regression_v2')
import torch
import torch.tensor
...
def test(config_file):#, label_wanted):
    ...
    Fine_Pnet = fine_net_class(
                    ...
                    fc_inc = 4*config_fine_pnet.get('base_feature_number', 16)*out_put_size,
    ...
    for ognb in [1,3,6,7]:
        ...
        with torch.no_grad():
            ...
            for ii_batch, sample_batch in enumerate(validLoader):
                ...
                coarse_support_cor = Coarse_Pnet(torch.from_numpy(support_batch).float())['position'].cpu().numpy().squeeze()
                fine_support_cor = Fine_Pnet(torch.from_numpy(support_batch).float())['position'].cpu().numpy().squeeze()
                ...
                query_cor = Coarse_Pnet(torch.from_numpy(query_batch).float())['position'].cpu().numpy().squeeze()#[10,3,D,W,H]
                full_size_relative_position = 300*np.tanh(coarse_support_cor-np.mean(query_cor, axis=0)) #
                ...
                for move_step in range(iter_move_num):
                    ...
                    query_cor = Fine_Pnet(torch.from_numpy(fine_query_batch).float())['position'].cpu().numpy().squeeze()#[6,3,D,W,H]
                    full_size_relative_position = 30*np.tanh(fine_support_cor-query_cor) # [6,3,D,W,H]
                    center_re_pos_patch = crop_patch_around_center(full_size_relative_position, r=[2,8,8])
                    center_query_patch = crop_patch_around_center(query_batch, r = [2,8,8])
                    center_point_relative_position = np.mean(center_re_pos_patch, axis=(2,3,4)) # [6, 3]
...
if __name__ == '__main__':
    config_file = str('/home/oem/home/data/zhaoqianfei/Relative_Distance_Regression/Relative_Distance_Regression_v2/config/test_position_full_size_double_stage.txt')
