    ...
    for ognb in [1,3,6,7]:
        ...
        with torch.no_grad():
            ...
            for ii_batch, sample_batch in enumerate(validLoader):
                ...
                cur_position[:,0] = np.minimum(np.maximum(cur_position[:,0], 24), 3*(ss[0]-8))
                cur_position[:,1] = np.minimum(np.maximum(cur_position[:,1], 48), ss[1]-48)
                cur_position[:,2] = np.minimum(np.maximum(cur_position[:,2], 48), ss[2]-48)
                ...
                for move_step in range(iter_move_num):
                    ...
                    relative_position = 30*np.tanh(fine_support_cor-query_cor) # [6,3]
                ...
                if show :
                    show_detection(sample_batch, support_position, initial_position=initial_position, predicted_position=cur_position[0])
