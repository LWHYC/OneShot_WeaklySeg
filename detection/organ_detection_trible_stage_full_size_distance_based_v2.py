...
def test(config_file):#, label_wanted):
    ...
    Middle_Pnet = torch.nn.DataParallel(Middle_Pnet, device_ids=[0,1,2,3]).cuda()
    ...
    Coarse_RD = Relative_distance(Coarse_Pnet,out_mode='position',distance_mode='tanh', \
                center_patch_size='all', distance_ratio=coarse_dis_ratio)
    Middle_RD = Relative_distance(Middle_Pnet,out_mode='fc_position',feature_refine=True,distance_mode='tanh', \
                center_patch_size='all', distance_ratio=middle_dis_ratio)
    Fine_RD = Relative_distance(Fine_Pnet,out_mode='fc_position',distance_mode='tanh', \
                center_patch_size='all', distance_ratio=fine_dis_ratio)
    ...
    for ognb in [1,3,6,7]:
        ...
        with torch.no_grad():
            ...
            for ii_batch, sample_batch in enumerate(validLoader):
                ...
                relative_position = Coarse_RD.cal_RD(query_patch=query_batch)['relative_position']
...
if __name__ == '__main__':
    config_file = str('/home/disk/LWH/Code/Relative_Distance_Regression_v2/config/test_position_full_size_trible_stage.txt')
