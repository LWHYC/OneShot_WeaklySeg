...
import sys
sys.path.append('/home/lwh/LWH/Position_Detection/Poistion_Detection')
import torch.tensor
...
def test(config_file, label_wanted):
    ...
    input = torch.rand((1,1,16,64,64)).cuda()
    t0 = time.time()
    feature = coarse_fnet(input)['8x']
    feature= coarse_pnet(feature,  feature)
    # feature = fine_fnet(feature)['8x']
    print(time.time()-t0)
        ...
        for ii_batch, sample_batch in enumerate(validLoader):
            ...
            for i in range(query_extreme_cor.shape[0]):
                ...
                for ii in range(patch_size.shape[0]):
                    ...
                    coarse_query_feature = coarse_fnet(query_batch)['8x']
                    fine_query_feature = fine_fnet(query_batch)['8x']
                    if ii ==0:
                        coarse_feature[0] = coarse_query_feature.view(coarse_query_feature.shape[0], -1)  # 生成
                        fine_feature[0] = fine_query_feature.view(fine_query_feature.shape[0], -1)
                    else:
                        coarse_feature[0] = torch.cat(
                            [coarse_feature[0], coarse_query_feature.view(coarse_query_feature.shape[0], -1)], dim=1)
                        fine_feature[0] = torch.cat(
                            [fine_feature[0], fine_query_feature.view(fine_query_feature.shape[0], -1)], dim=1)
...
if __name__ == '__main__':
    ...
    config_file = str('/home/lwh/LWH/Position_Detection/Poistion_Detection/config/test_position_double_stage.txt')
