...
import sys
sys.path.append('/media/lwh/09b9fdae-489a-484b-8cc7-a0a31e663bdd/代码/DeepLearning/Pytorch-project/Poistion_Detection')
import torch.tensor
...
def test(config_file, label_wanted):
    ...
    with torch.no_grad():
        ...
        for ii_batch, sample_batch in enumerate(validLoader):
            ...
            for i in range(query_extreme_cor.shape[0]):
                ...
                for ii in range(patch_size.shape[0]):
                    query_batch = sample_batch['image_1'][query_position[0] - patch_size[ii, 0] // 2:query_position[0] + patch_size[ii,0] // 2,
                                query_position[1] - patch_size[ii, 1] // 2:query_position[1] + patch_size[ii, 1] // 2,
                                query_position[2] - patch_size[ii, 2] // 2:query_position[2] + patch_size[ii, 2] // 2]
                    query_batch = torch.from_numpy(query_batch).unsqueeze(dim=0).unsqueeze(dim=0).float()
                    query_feature = fnet(query_batch)['8x']
                    if ii ==0:
                        feature[0] = query_feature.view(query_feature.shape[0], -1)  # 生成
                    else:
                        feature[0] = torch.cat([feature[0], query_feature.view(query_feature.shape[0], -1)], dim=1)
                ...
                '''
                    ...
                    for iii in range(iter_move_num):
                        for iiii in range(patch_size.shape[0]):
                            ...
                            if iiii == 0:
                                feature[1] = answer_feature.view(query_feature.shape[0], -1)  # 生成
                            else:
                                feature[1] = torch.cat([feature[1], answer_feature.view(answer_feature.shape[0], -1)],
                                                       dim=1)
                            predic = pnet(feature[0], feature[1])
                            distance = predic['distance'].cpu().data.numpy().squeeze()
                            target_feature = predic['feature_0'].cpu().data.numpy().squeeze()
                            cur_position = np.round(cur_position + distance).astype(np.int16)
                            print('cur position',cur_position)
                            cur_position=get_position_feature_based(target_feature, sample_batch['image_1'], cur_position=cur_position
                                                                  ,fnet=fnet, pnet=pnet, windowSize=patch_size[iiii],stepSize=2)
                            print('moved position', cur_position)
                    predic_position.append( cur_position)
                predic_position = cal_average_except_minmax(predic_position)
                ...
                if show :
                    show_detection(sample_batch, query_position, initial_position=initial_position, predicted_position=predic_position)
            t2 = time.time()
        ...
        print('mean error:', np.asarray([1.5,0.5,0.5])*np.sum(error_sum, axis=0)/len(
...
if __name__ == '__main__':
    label_wanted = int(input('label_wanted:'))
