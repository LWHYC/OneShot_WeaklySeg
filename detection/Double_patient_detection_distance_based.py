...
def test(config_file):
    ...
    with torch.no_grad():
        ...
        for ii_batch, sample_batch in enumerate(validLoader):
            ...
            target_batch, target_position= sample['random_crop_image_0'].cuda().unsqueeze(0), sample['random_position_0']
            target_position = np.int16(target_position).squeeze()
            target_feature = fnet(target_batch)
            predicted_position = np.asarray([0,0,0])
            while move_step < 30:
                sample = random_crop(sample_batch)
                sample = to_tensor(sample)
                cur_batch, cur_position = sample[
                                                                                       'random_crop_image_1'].cuda().unsqueeze(
                                                                                       0), sample[
                                                                                       'random_position_1']
                # print(cur_position)
                cur_position = np.int16(cur_position).squeeze()
                initial_position = cur_position
                cur_feature = fnet(cur_batch)
                predic = np.int16(pnet(target_feature, cur_feature).cpu().data.numpy().squeeze())
                #print(predic)
                cur_position = cur_position+ predic
                predicted_position =  predicted_position + cur_position
                move_step+=1
                # print('target position', target_position, 'cur position', cur_position)


            # cur_batch = sample_batch['image_1'][cur_position[0]//3:cur_position[0]//3+test_patch_size[0], cur_position[1]:cur_position[1]+test_patch_size[1],
            #           cur_position[2]:cur_position[2]+test_patch_size[2]]
            # cur_batch = torch.from_numpy(cur_batch).cuda().unsqueeze(0).unsqueeze(0).float()
            predicted_position //= move_step
            initial_position[0]//=3
            target_position[0]//=3
            predicted_position[0]//=3
            # initial_position -= np.asarray(test_patch_size)//2
            # target_position -= np.asarray(test_patch_size) // 2
            # predicted_position -= np.asarray(test_patch_size) // 2
            print('target position', target_position, 'initial position',initial_position, 'predicted position', predicted_position)
            ...
            plt.plot(initial_position[2], initial_position[1], '*', c='r')
