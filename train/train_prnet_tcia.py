...
def train(config_file):
    ...
    if config_train.get('parallel_training'):
        prnet = torch.nn.DataParallel(prnet, device_ids=config_train.get('device_ids')).cuda()
    else:
        prnet = prnet.cuda()
    ...
    if config_train['load_weight']:
        if os.path.isfile(config_train['prnet_load_path']):
            ...
            if config_train['prnet_load_path'].endswith('.tar'):
            ...
            elif config_train['prnet_load_path'].endswith('.pkl'):
                prnet_weight = torch.load(config_train['prnet_load_path'], map_location=lambda storage, loc: storage) # position net
                prnet.load_state_dict(prnet_weight)
            print("=> loaded checkpoint '{}' ".format(config_train['prnet_load_path']))
        else:
            print("=> no checkpoint found at '{}'".format(config_train['prnet_load_path']))
    ...
    for epoch in range(start_it, config_train['maximal_epoch']): 
        ...
        for i_batch, sample_batch in enumerate(trainLoader):
            ...
            predic_0 = prnet(img_batch0, loss_func)
            predic_1 = prnet(img_batch1, loss_func)
        ...
        if  epoch % config_train['test_step']==0:
            ...
            'save model'
            filename = config_train['prnet_save_name'] + "_cur_{0:}.tar".format(epoch_loss['valid_loss'])
            if epoch_loss['valid_loss'] < best_loss:
                best_loss =epoch_loss['valid_loss']
                is_best = True
                bestname = config_train['prnet_save_name'] + "_{0:}.tar".format(epoch_loss['valid_loss'])
            else:
                is_best = False
                bestname = None
...
if __name__ == '__main__':
    config_file = '../config/train_prnet_tcia.txt'
